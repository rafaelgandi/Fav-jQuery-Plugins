...
function Boot() {
	...
	this._inCacheThenEval = function (_jsPath) {
		var cache = localStorage.getItem(key+_jsPath);
		if (!!cache) {
			jQuery.globalEval(cache);
			if (typeof console !== 'undefined') {console.log(_jsPath+' has been loaded');}
			return true;
		}
		return false;
	};
	this._makeCache = function (_js) {			
		setTimeout(function () {
			jQuery.get(_js, function (res) {
				try {
					localStorage.setItem(key+_js, res);
				}catch(e){}		
